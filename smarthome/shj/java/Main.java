...
public class Main {
    ...
    private static void addDeviceInteractive() {
        ...
        int pw = Integer.parseInt(sc.nextLine().trim());
        switch (t) {
            case "1":
                System.out.print("Яркость: ");
                int b = Integer.parseInt(sc.nextLine().trim());
                System.out.print("Цвет: ");
                String col = sc.nextLine().trim();
                system.addDevice(new Light(id, name, loc, pw, b, col));
                break;
            case "2":
                System.out.print("Температура: ");
                double temp = Double.parseDouble(sc.nextLine().trim());
                system.addDevice(new Thermostat(id, name, loc, pw, temp));
                break;
            case "3":
                system.addDevice(new DoorLock(id, name, loc, pw));
                break;
            case "4":
                system.addDevice(new Camera(id, name, loc, pw));
                break;
            case "5":
                system.addDevice(new Sprinkler(id, name, loc, pw));
                break;
            case "6":
                System.out.print("Позиция: ");
                int pos = Integer.parseInt(sc.nextLine().trim());
                system.addDevice(new Blinds(id, name, loc, pw, pos));
                break;
            case "7":
                system.addDevice(new SmokeDetector(id, name, loc, pw));
                break;
            case "8":
                system.addDevice(new EnergyMeter(id, name, loc, pw));
                break;
            default:
    ...
    private static void configureDeviceInteractive() {
        ...
        String cmd = sc.nextLine().trim();
        if (cmd.equals("1")) d.turnOn();
        else if (cmd.equals("2")) d.turnOff();
    ...
    private static void scenariosMenu() {
        while (true) {
            ...
            if (c.equals("1")) {
                ...
                String name = sc.nextLine().trim();
                system.addScenario(new Scenario(id, name));
    ...
    private static void schedulerMenu() {
        while (true) {
            ...
            if (c.equals("1")) {
                ...
                try {
                    system.getScheduler().add(id, s, LocalDateTime.parse(when));
                } catch (Exception e) {
